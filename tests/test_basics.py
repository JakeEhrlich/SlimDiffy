...
def test_from_to_value():
    ...
    test_cases = [
        ...
        {'x': [1, 2], 'y': {'z': 3}},
        [{'a': [1, 2]}, (3, {'b': 4})],
        ExampleDataClass(x=1, y={'test': 5}, z=[7,8]),
        np.array([1, 2, 3])
    ]
...
class ExampleDataClass:
    x: int = pt.static_field()
    y: dict
    z: list

def test_dataclass():
    obj = ExampleDataClass(x=1, y={'a': 1}, z=[1,2])
...
def test_static_fields():
    ...
    dx = dh(x, slope, intercept)
    assert abs(dx - slope) < 1e-6


def test_general_transpose():
    test_cases = [
        # Basic 2D transpose
        ((2, 3), (1, 0)),
    ...
    for shape, axes in test_cases:
        ...
        # Compare against numpy transpose
        result = f(x, axes)
...
def test_general_dot():
    test_cases = [
        # Basic matrix multiply - contract last dim of lhs with first dim of rhs
        ((2, 3), (3, 4), (1,), (0,), (), (),
         lambda a, b: a @ b),

        # Contract inner dimensions with batch dim
        ((5, 2, 3), (5, 3, 4), (2,), (1,), (0,), (0,),
         lambda a, b: np.array([a[i] @ b[i] for i in range(5)])),

        # Multiple contracting dimensions
        ((2, 3, 4), (4, 3, 5), (1, 2), (1, 0), (), (),
         lambda a, b: np.tensordot(a, b, ((1, 2), (1, 0)))),

        # Multiple batch dimensions
        ((2, 3, 4, 5), (2, 3, 5, 6), (3,), (2,), (0, 1), (0, 1),
         lambda a, b: np.array([[a[i,j] @ b[i,j] for j in range(3)] for i in range(2)])),

        # No contracting dimensions (outer product)
        ((2, 3), (4, 5), (), (), (), (),
         lambda a, b: np.tensordot(a, b, 0)),

        # Contract all dimensions (inner product)
        ((3, 4), (4, 3), (0, 1), (1, 0), (), (),
    ...
    for lhs_shape, rhs_shape, lhs_c, rhs_c, lhs_b, rhs_b, ref_impl in test_cases:
        x = np.random.randn(*lhs_shape)
        y = np.random.randn(*rhs_shape)

        # Compare against reference implementation
        result = f(x, y, lhs_c, rhs_c, lhs_b, rhs_b)
        expected = ref_impl(x, y)
